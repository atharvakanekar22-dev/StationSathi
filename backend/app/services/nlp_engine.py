...
class DomainNLPEngine:
    ...
    def interpret_query(self, req: AssistantQueryRequest) -> AssistantQueryResponse:
        ...
        if "east entrance" in q or "east concourse" in q:
            detected_origin = "node_entrance_east"
        elif "west entrance" in q or "west concourse" in q:
        ...
        avoid_stairs = any(phrase in q for phrase in [
            ...
            "avoiding stairs", "wheelchair", "accessible", "lift only"
        ])
        prefer_elevator = "elevator" in q or "lift" in q

        # 2. Check for Platform destination
        platform_match = re.search(r'platform\s*([0-9]+|one|two|three|four|five|six|seven|eight)', q)
        target_platform_num = None
        if platform_match:
            val = platform_match.group(1)
            mapping = {"one": "1", "two": "2", "three": "3", "four": "4", "five": "5", "six": "6", "seven": "7", "eight": "8"}
            target_platform_num = mapping.get(val, val)
        ...
        if target_platform_num:
            destination_node_id = f"node_pf{target_platform_num}"
            preference = "avoid_stairs" if avoid_stairs else ("prefer_elevator" if prefer_elevator else "shortest")
            
            origin_to_use = active_origin or "node_entrance_east"
            
            explanation = (
                f"Detected route query to Platform {target_platform_num} with "
                f"{'stair avoidance constraint' if avoid_stairs else 'standard routing'}."
            )
            
            answer_text = (
                f"Calculated navigation route to Platform {target_platform_num} at Dadar Central. "
                f"{'Stairs avoided via accessible elevator.' if avoid_stairs else 'Shortest walkable path calculated.'}"
            )

            # Get matched facility
            pf_facilities = self.repository.get_facilities_by_station(station_id, category="platform")
            matched = [f for f in pf_facilities if f.node_id == destination_node_id]
        ...
        if category:
            ...
            if "near" in q or "nearest" in q or "closest" in q:
                if not active_origin:
                    return AssistantQueryResponse(
                        ...
                        category=category,
                        explanation=f"Found {len(matched_facilities)} {category} facility record(s). Distance ranking requires your current landmark.",
                        answer_text=(
                            f"Mapped {len(matched_facilities)} {category.replace('_', ' ')} location(s) at {station_id.title()}. "
            ...
            return AssistantQueryResponse(
                interpreted_intent="facility_search",
                category=category,
                explanation=f"Found {len(matched_facilities)} {category.replace('_', ' ')} location(s) in {station_id.title()}.",
                answer_text=f"Found {len(matched_facilities)} mapped {category.replace('_', ' ')} facility record(s) at {station_id.title()}.",
        ...
        if all_facs:
            return AssistantQueryResponse(
                ...
                answer_text=f"Showing {len(all_facs)} matching location(s) for '{req.query}'.",
                suggested_action="show_facilities",
        ...
        return AssistantQueryResponse(
            ...
            answer_text=(
                f"StationSathi provides indoor facility navigation for {station_id.title()}. "
                "You can search for washrooms, shoe-polishing stands, ticket counters, drinking water, or platforms."
            ),
            suggested_action="show_facilities",
            matched_facilities=[],
